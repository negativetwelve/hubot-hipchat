  ...
  run: ->
    ...
    connector.onConnect =>
      ...
      saveUsers = (users) =>
        ...
        # for user in users
          # user_id = @userIdFromJid user.jid
          # user.id = user_id
          # userForId will not overwrite an existing user
          # @robot.brain.data.users[user_id] = user
      ...
      changePresence = (PresenceMessage, user_jid, room_jid, current_name) =>
      ...
      connector.onEnter (user_jid, room_jid, current_name) =>
        changePresence EnterMessage, user_jid, room_jid, current_name
